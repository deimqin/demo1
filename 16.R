...
# Function xử lý dữ liệu năm 2016
process_data_2016 <- function(file_path, columns) {
  read_dta(file_path) %>% 
    mutate(h_code = str_c(add_zero(tinh), add_zero(huyen), add_zero(xa), add_zero(diaban), add_zero(hoso))) %>% 
    filter(h_code %in% full_16_14_12) %>%
...
# Dữ liệu y tế năm 2016
muc3a_2016 <- process_data_2016(
  "C:/Users/My PC/VHLSS 2016/Bang hoi VHLSS 2016/Data VHLSS 2016/Muc3B.dta",
  c("m3c5a", "m3c5b", "m3c6a", "m3c6b"))

muc3b_2016 <- process_data_2016(
  "C:/Users/My PC/VHLSS 2016/Bang hoi VHLSS 2016/Data VHLSS 2016/Muc3A.dta",
  c("m3c11", "m3c12a", "m3c12b", "m3c13", "m3c14", "m3c15")
)

# Thu nhập năm 2016
income_2016 <- process_data_2016(
  "C:/Users/My PC/VHLSS 2016/Bang hoi VHLSS 2016/Data VHLSS 2016/Ho3.dta", 
  c("thubq", "thunhap")
)

# Tổng thu nhập năm 2016
sumincome_2016 <- process_data_2016(
  "C:/Users/My PC/VHLSS 2016/Bang hoi VHLSS 2016/Data VHLSS 2016/Ho2.dta", 
  c("m4atn", "m4dtn")
)

# Lấy mã tỉnh từ năm 2016
tinh_2016 <- data_food_consumption %>% 
  filter(h_code %in% full_16_14_12, year == 2016) %>%
  select(h_code, tinh)

# 3: MERGE 
food_consumption_16 <- target_2016 %>% 
  left_join(tinh_2016, by = "h_code") %>%
  left_join(household_size_2016, by = "h_code") %>%
  left_join(household_heads_2016, by = "h_code") %>%
  left_join(muc3a_2016, by = "h_code") %>%
  left_join(muc3b_2016, by = "h_code") %>%
  left_join(income_2016, by = "h_code") %>%
  left_join(sumincome_2016, by = "h_code")

# Đổi tên biến
food_consumption_16 <- food_consumption_16 %>%
  rename(
    # Target
    actual_food_2016 = total_food_con_2016,
  ...
  relocate(actual_food_2016, .after = last_col())

# Xuất file
write.csv(food_consumption_16, "C:/Users/My PC/food_consumption_16.csv", row.names = FALSE)
